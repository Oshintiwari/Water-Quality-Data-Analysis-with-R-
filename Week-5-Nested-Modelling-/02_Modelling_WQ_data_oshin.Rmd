...
```{r}
q_data$siteid <- paste0('USGS-', q_data$siteid)

colnames(q_data)[colnames(q_data) == "Date"] <- "date"

joined_data <- inner_join(wq, q_data, by = c("date", "siteid"))
...
```{r}
selected_site <- 'USGS-09034500'
selected_parameter <- 'Calcium'

selected_data <- joined_data %>%
  filter(siteid == selected_site, parameter == selected_parameter)

ggplot(selected_data, aes(x = Flow, y = conc)) +
  geom_point() +
  labs(title = paste("Discharge vs", selected_parameter, "for Site", selected_site),
       x = "Discharge", y = selected_parameter)
...
grouped_data <- grouped_data %>%
  mutate(tidy_mods = map(mods, tidy))
...
ggplot(tidy_data, aes(x = term, y = estimate, color = basin)) +
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error), width = 0.2) +
  labs(title = "Linear Model Summaries for Discharge vs Concentration",
       x = "Term", y = "Estimate", color = "Basin") +
...
The furrr::map function is similar to purrr::map, but it provides a parallel version of the function. It's useful when we want to parallelize the code and speed up the execution of multiple independent tasks. furrr allows to easily switch between sequential and parallel execution without changing the code structure.


When would you want to use this `furrr` function?

We might want to use furrr::map when we have a large dataset, and each element of the dataset can be processed independently. By parallelizing the tasks, we can take advantage of multiple cores or nodes, leading to faster computations.