...
```{r}
raw_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes={ROMO}&startMonth={January}&startYear={2010}&endMonth={December}&endYear={2021}")
...
```{r}
unit_visitation <- function( park_unit, start_month, start_year, end_month, end_year) {
...
```{r}
data_ROMO <- unit_visitation( "ROMO", "1", "2010", "12", "2021")
data_ROMO
```

```{r}
data_EVER <- unit_visitation( "EVER", "1", "2010", "12", "2021")
data_EVER
```


```{r}
data_THRO <- unit_visitation( "THRO", "1", "2010", "12", "2021")
data_THRO
```
...
```{r}
romo_data <- unit_visitation(park_unit = "ROMO", start_year = "1980", end_year = "2021") 
romo_data
```
```{r}
data_EVER <- unit_visitation( park_unit = "EVER", start_year = "1980", end_year = "2021")
data_EVER
```

```{r}
data_THRO <- unit_visitation( park_unit = "THRO", start_year = "1980", end_year = "2021")
data_THRO
```
...
for(i in 1:length(parks)){
  ...
  output_parks[[i]] <-
    unit_visitation(park_unit = parks[i], start_year = "1980", end_year = "2021")
  
}
```

```{r}
summary(output_parks)
```

```{r}
multi_parks <- dplyr::bind_rows(output_parks)
multi_parks
```
...
```{r}
identical(output_parks, output_map_parks)
```
